...
describe("pet visual shell", () => {
  ...
  it("allocates a scaled sprite slot and excludes overlay selectors from the shape reporter", async () => {
    ...
    const [entry, reporter, app] = await Promise.all([
      ...
      readFile(join(process.cwd(), "src", "renderer", "app", "App.tsx"), "utf8"),
    ]);
    ...
    expect(app).toContain(".then(() => setQuickViewVisible(true))");
  });
});
