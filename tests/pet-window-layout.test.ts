...
describe("pet window bounds", () => {
  it("links 50%, 100%, and 200% visual sizes to non-cropping compact bounds", () => {
    expect(bounds({ scalePercent: 50 })).toEqual({ x: 1620, y: 720, width: 300, height: 360 });
    expect(bounds({ scalePercent: 100 })).toEqual({ x: 1620, y: 720, width: 300, height: 360 });
    expect(bounds({ scalePercent: 200 })).toEqual({ x: 1428, y: 528, width: 492, height: 552 });
  });

  it("preserves left-bottom and right-bottom anchors", () => {
    expect(bounds({ scalePercent: 200, anchor: "left-bottom" })).toMatchObject({ x: 0, y: 528 });
  ...
  it("recomputes for a different frame, expanded cards, and physical display compensation", () => {
    ...
    expect(atlas.width).toBeGreaterThanOrEqual(300);
    expect(atlas.height).toBeGreaterThanOrEqual(360);
